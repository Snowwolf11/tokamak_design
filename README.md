...
are as important as function signatures in a software library.
...
Every execution creates a RUN DIRECTORY:
...
Creates the run directory and initializes results.h5.
...
This stage guarantees reproducibility.
...
Defines everything STATIC and PLASMA-INDEPENDENT:
...
This defines the MACHINE CONTRACT.
Outputs are treated as immutable thereafter.
...
Solves the Grad–Shafranov equation inside the prescribed LCFS:
...
Fits PF coil currents so the VACUUM FLUX matches the target boundary.
...
Generates standardized plots by reading only results.h5.

Works for partial runs and writes to:
  run_dir/figures/
...
Convenience wrapper to run stages sequentially with one command.
...
Run up to a fixed-boundary equilibrium:
...
The current implementation is a carefully designed foundation for this growth.
