...
from tokdesign.geometry.grids import grid_spacing
from tokdesign.geometry.plasma_boundary import boundary_kappa_delta, ensure_closed_polyline

from tokdesign.physics.gs_profiles import profiles_from_dict, normalize_psi
from tokdesign.physics.gs_solve_fixed import solve_fixed_boundary_gs
from tokdesign.physics.fields import compute_BR_BZ_from_psi, compute_Bphi_from_F
from tokdesign.physics.derived import (
    compute_Ip,
    compute_beta_p,
    compute_li,
    approx_q_profile,
...
def main() -> None:
    ...
    if not args.overwrite:
        with open_h5(results_path, "r") as h5:
            if ("/equilibrium/psi" in h5) or ("/fields/BR" in h5) or ("/derived/Ip" in h5):
                raise RuntimeError(
                    "It looks like /equilibrium, /fields, or /derived already exist in results.h5.\n"
    ...
    with open_h5(results_path, "r+") as h5:
        ...
        h5_ensure_group(h5, "/fields")
        h5_ensure_group(h5, "/derived")

        # equilibrium
        h5_write_array(h5, "/equilibrium/psi", psi, attrs={"units": "Wb_per_rad"})
        h5_write_scalar(h5, "/equilibrium/psi_axis", psi_axis, attrs={"units": "Wb_per_rad"})
        h5_write_scalar(h5, "/equilibrium/psi_lcfs", float(psi_lcfs), attrs={"units": "Wb_per_rad"})
        h5_write_array(h5, "/equilibrium/p_psi", p_psi, attrs={"units": "Pa"})
        h5_write_array(h5, "/equilibrium/F_psi", F_psi, attrs={"units": "T*m"})
        h5_write_array(h5, "/equilibrium/plasma_mask", plasma_mask.astype(np.uint8),
                       attrs={"note": "1=inside plasma"})
        h5_write_array(h5, "/equilibrium/jphi", jphi, attrs={"units": "A/m^2"})

        # fields
        h5_write_array(h5, "/fields/BR", BR, attrs={"units": "T"})
        h5_write_array(h5, "/fields/BZ", BZ, attrs={"units": "T"})
        h5_write_array(h5, "/fields/Bphi", Bphi, attrs={"units": "T"})

        # derived
        h5_write_scalar(h5, "/derived/Ip", float(Ip), attrs={"units": "A"})
        h5_write_scalar(h5, "/derived/beta_p", float(beta_p), attrs={"dimensionless": 1})
        h5_write_scalar(h5, "/derived/li", float(li), attrs={"dimensionless": 1})
        h5_write_scalar(h5, "/derived/kappa", float(kappa), attrs={"dimensionless": 1})
        h5_write_scalar(h5, "/derived/delta", float(delta), attrs={"dimensionless": 1})
        h5_write_array(h5, "/derived/q_profile", q_profile, attrs={"columns": "psin,q"})

        # provenance/debug attrs
        h5_write_dict_as_attrs(h5, "/equilibrium", {
